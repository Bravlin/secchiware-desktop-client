...
export default {
    ...
    methods: {
        async deleteSelectedPackages() {
            if (this.selected.length == 0)
                this.showErrorModal('You must select at least one package.');
            else {
                let confirmed = await this.showDangerousOperationConfirmationModal(
                ...
                if (confirmed) {
                    ...
                    for (pack of this.selected) {
                        ...
                        try {
                            ...
                            this.showErrorModal(
                                'Something went wrong when trying to contact the Command and '
                                + 'Control server to delete the packages. Please, verify that the '
